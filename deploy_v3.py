...
commands = [
    ...
    "cd /var/www/smarterp/server && npx prisma db push --accept-data-loss 2>&1", # Activate new AuditLog table
    "pm2 restart smarterp-backend --no-color 2>&1",
    "echo DEPLOY_SUCCESS"
]

log = open("deploy_v3_log.txt", "w", encoding='utf-8')
