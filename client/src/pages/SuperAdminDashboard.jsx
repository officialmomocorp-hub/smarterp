    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-gray-500 mt-1">Hello Super Admin, here is the global view of Smarterp.</p>
        </div>
        <div className="hidden md:flex gap-4">
           <button className="px-4 py-2 bg-primary-600 text-white rounded-xl shadow-lg shadow-primary-200 hover:bg-primary-700 transition-all font-medium">Add New School</button>
           <button className="px-4 py-2 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-all font-medium">Export Reports</button>
        </div>
      </div>
             ...
             <div className="space-y-4">
                {data.recentSchools.map(school => (
                  <div key={school.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                    ...
                    <span className="text-xs font-medium text-gray-400">{new Date(school.createdAt).toLocaleDateString('en-IN', {day:'2-digit', month:'short'})}</span>
                  </div>
                ))}
                <button className="w-full mt-4 py-3 text-sm font-bold text-primary-600 bg-primary-50 rounded-xl hover:bg-primary-100 transition-colors">
                   View All Schools
                </button>
             </div>
          </div>
