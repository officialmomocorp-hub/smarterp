    ...
    <div className="space-y-6">
      ...
      <div className="flex bg-gray-100 p-1 rounded-xl w-fit">
        <button 
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'inventory' ? 'bg-white text-[#409CFF] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
        >
          <Bed className="w-4 h-4" /> Room Inventory
        </button>
        <button 
          onClick={() => setActiveTab('billing')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'billing' ? 'bg-white text-[#409CFF] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
        >
             ...
             <div key={hostel.id} className="card">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {hostel.rooms?.map(room => (
                     <div key={room.id} className="p-4 border border-[rgba(84,84,88,0.36)] rounded-xl hover:bg-gray-50 transition-all">
                        ...
                        <div className="space-y-1">
                           {room.allotments?.map(allot => (
                             <div key={allot.id} className="flex items-center gap-2 text-xs">
                                <div className="w-4 h-4 rounded-full bg-[#0A84FF]/15 flex items-center justify-center text-[#409CFF] font-bold">
        ...
        <div className="space-y-6">
           <div className="card bg-gradient-to-br from-primary-600 to-primary-800 text-white">
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="bg-[#2c2c2e] p-3 rounded-lg border border-white/20">
                    <p className="text-xs opacity-80 uppercase font-bold">Total Space</p>
                    <p className="text-2xl font-bold">{hostels.reduce((acc, h) => acc + h.rooms?.reduce((r_acc, r) => r_acc + r.capacity, 0), 0)}</p>
                 </div>
                 <div className="bg-[#2c2c2e] p-3 rounded-lg border border-white/20">
                    <p className="text-xs opacity-80 uppercase font-bold">Occupied</p>
                    <p className="text-2xl font-bold">{hostels.reduce((acc, h) => acc + h.rooms?.reduce((r_acc, r) => r_acc + r.occupied, 0), 0)}</p>
                 </div>
              </div>
              <button className="w-full mt-6 py-2 px-4 bg-white text-[#409CFF] font-bold rounded-lg text-sm hover:bg-opacity-90 transition-all">
           ...
           <div className="card">
              <h4 className="font-bold text-white mb-4 flex items-center gap-2">
                 <CreditCard className="w-4 h-4 text-[#0A84FF]" /> Pending Receipts
              </h4>
              <div className="divide-y divide-gray-100">
                 {[1, 2, 3].map(i => (
                    <div key={i} className="py-3 flex justify-between items-center group cursor-pointer">
                       <div>
                          <p className="text-sm font-medium text-white">Student #{1000 + i}</p>
                          <p className="text-xs text-gray-500">March Mess Fee</p>
                       </div>
                       <p className="text-sm font-bold text-amber-600">₹2,400</p>
                    </div>
                 ))}
              </div>
              <button className="w-full mt-4 text-xs text-[#0A84FF] font-bold hover:underline">View All Billing</button>
           </div>
        </div>
      </div>
    </div>
